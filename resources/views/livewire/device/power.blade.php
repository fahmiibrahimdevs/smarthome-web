<div>
    <section class="section">
        <div class="section-header tw-rounded-none lg:tw-rounded-lg tw-shadow-md tw-shadow-gray-300 px-4">
            <h1 class="tw-text-lg mb-1">Configure Device - POWER</h1>
        </div>
                ...
                <div class="card-body px-0">
                    ...
                    <div class="table-responsive tw-max-h-[500px]">
                        <table>
                            ...
                            <tbody>
                                @foreach ($data->groupBy('name_location') as $location => $locationData)
                                <tr class="tw-tracking-wider">
                                    <td class="tw-font-bold" colspan="5">
                                        {{ $location }}
                                    </td>
                                </tr>
                                @foreach ($locationData->groupBy('name_room') as $room => $roomData)
                                <tr class="tw-tracking-wider">
                                    <td></td>
                                    <td class="tw-font-bold" colspan="5">
                                        {{ $room }}
                                ...
                                @foreach ($roomData as $res)
                                <tr class="text-center">
                                    <td></td>
                                    <td></td>
                                    <td class="text-left">{{ $res['name_feature'] }}</td>
                                    <td class="text-left">{{ $res['name_device'] }}</td>
                                    <td class="text-left">{{ $res['topic'] }}</td>
                                    <td>
                                        <button class="btn btn-primary" wire:click="edit({{ $res['id'] }})"
                                            data-toggle="modal" data-target="#ubahDataModal">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button class="btn btn-danger"
                                            wire:click.prevent="deleteConfirm({{ $res['id'] }})">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="tambahDataModalLabel">Add Data</h5>
                ...
                <form>
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control tw-rounded-lg" name="id_location" id="id_location"
                                        wire:model='id_location'>
                                        <option value="" disabled>-- Select Option --</option>
                                        @foreach ($locations as $location)
                                        <option value="{{ $location->uuid }}" selected>
                                ...
                                <div class="form-group">
                                    ...
                                    <div wire:ignore>
                                        <select class="form-control tw-rounded-lg" name="id_feature" id="id_feature"
                                            ...
                                            <option value="{{ $feature->uuid }}" selected>{{ $feature->name_feature }}
                                            </option>
                                            @endforeach
                                        </select>
                                    </div>
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="active">Message for Active</label>
                                    <input type="text" class="form-control tw-rounded-lg" name="active" id="active"
                                        wire:model='active'>
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="inactive">Message for Inactive</label>
                                    <input type="text" class="form-control tw-rounded-lg" name="inactive" id="inactive"
                                        wire:model='inactive'>
                ...
                <form>
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="edit-id_location">Location</label>
                                    <select class="form-control tw-rounded-lg" name="edit-id_location"
                                        id="edit-id_location" wire:model='id_location'>
                                        <option value="" disabled>-- Select Option --</option>
                                        @foreach ($locations as $location)
                                        <option value="{{ $location->uuid }}" selected>
                                            {{ $location->name_location }}</option>
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="edit-id_room">Room</label>
                                    <select class="form-control tw-rounded-lg" name="edit-id_room" id="edit-id_room"
                                        wire:model='id_room'>
                                        <option value="" disabled>-- Select Option --</option>
                                        @foreach ($rooms as $room)
                                        <option value="{{ $room->uuid }}" selected>{{ $room->name_room }}</option>
                            ...
                            <div class="col-lg-12">
                                <div class="form-group">
                                    <label for="id_feature">Feature</label>
                                    <div wire:ignore>
                                        <select class="form-control tw-rounded-lg" name="id_feature"
                                            id="edit-id_feature" wire:model='id_feature'>
                                            @foreach ($features as $feature)
                                            <option value="{{ $feature->uuid }}" selected>{{ $feature->name_feature }}
                                            </option>
                                            @endforeach
...
<script>
    $(document).ready(function () {
        ...
        Livewire.on('select2:init', function () {
            $('#id_room').select2();
            $('#id_location').select2();
            $('#edit-id_room').select2();
            $('#edit-id_location').select2();
